...
static gboolean
file_is_stdin (ChicleFileMapping *file_mapping)
{
...
static void
free_file_data (ChicleFileMapping *file_mapping)
{
...
static gint
cache_stdin (ChicleFileMapping *file_mapping, GError **error)
{
    gpointer buf = NULL;
...
static gint
open_file (ChicleFileMapping *file_mapping, GError **error)
{
    gint fd;
...
static gboolean
ensure_open_file (ChicleFileMapping *file_mapping)
{
...
static gboolean
map_or_read_file (ChicleFileMapping *file_mapping)
{
...
ChicleFileMapping *
chicle_file_mapping_new (const gchar *path)
{
    ChicleFileMapping *file_mapping;

    file_mapping = g_new0 (ChicleFileMapping, 1);
...
void
chicle_file_mapping_destroy (ChicleFileMapping *file_mapping)
{
...
gboolean
chicle_file_mapping_open_now (ChicleFileMapping *file_mapping, GError **error)
{
    if (file_mapping->data || file_mapping->fd >= 0)
        return TRUE;
...
const gchar *
chicle_file_mapping_get_path (ChicleFileMapping *file_mapping)
{
    return file_mapping->path;
}

gconstpointer
chicle_file_mapping_get_data (ChicleFileMapping *file_mapping, gsize *length_out)
{
    if (!file_mapping->data)
...
gboolean
chicle_file_mapping_taste (ChicleFileMapping *file_mapping, gpointer out, goffset ofs, gsize length)
{
...
gssize
chicle_file_mapping_read (ChicleFileMapping *file_mapping, gpointer out, goffset ofs, gsize length)
{
    if (!ensure_open_file (file_mapping))
        return FALSE;
...
gboolean
chicle_file_mapping_has_magic (ChicleFileMapping *file_mapping, goffset ofs, gconstpointer data, gsize length)
{
