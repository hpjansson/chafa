...
static void
clear_current_src (PathQueue *path_queue)
{
...
static void
check_src_end (PathQueue *path_queue)
{
...
static void
open_current_stream (PathQueue *path_queue)
{
    gint fd;
...
static gboolean
ensure_current_src (PathQueue *path_queue)
{
    for (;;)
    {
        if (!path_queue->current_src)
...
static gboolean
pop_stream_path_token (PathQueue *path_queue)
{
    gint result;
...
PathQueue *
path_queue_ref (PathQueue *path_queue)
{
    gint refs;
...
void
path_queue_unref (PathQueue *path_queue)
{
    gint refs;
...
void
path_queue_push_path (PathQueue *path_queue, const gchar *path)
{
    PathSource *src;
...
void
path_queue_push_path_list_steal (PathQueue *path_queue, GList *path_list)
{
    PathSource *src;
...
void
path_queue_push_stream (PathQueue *path_queue, const gchar *stream_path,
...
void
path_queue_wait (PathQueue *path_queue)
{
...
gchar *
path_queue_pop (PathQueue *path_queue)
{
    gchar *path = NULL;

    for (;;)
    {
        path = path_queue_try_pop (path_queue);
        if (path)
            break;

        if (path_queue_is_empty (path_queue))
            break;

        path_queue_wait (path_queue);
    }
...
gint
path_queue_get_length (PathQueue *path_queue)
{
    return g_queue_get_length (path_queue->queue);
}

gboolean
path_queue_is_empty (PathQueue *path_queue)
{
    if (!ensure_current_src (path_queue))
        return TRUE;
...
gint
path_queue_get_n_processed (PathQueue *path_queue)
{
